...
//			System.out.println(start_idx +" "+ end_idx);
			...
			if(group_idx1 != -1 && group_idx2 != -1) {
				matching += (nums.get(group_idx1) * nums.get(group_idx2));
		...
		for(;start_idx <= end_idx; start_idx++) {
			matching += nums.get(start_idx);
		}
		System.out.println(matching);
	}
	
	private static int multiple_benefit(int num1, int num2) {
